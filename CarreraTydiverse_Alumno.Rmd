...
library(pacman)
p_load(dplyr,lubridate, tidyr,ggplot2, readr, stringr)
...
```{r}
cran3 <- cran3 %>%
  mutate(size_mb, size_gb=size/2^10)
```
...
```{r}
cran3 <- cran3 %>%
  mutate(size, correct_size=size+1000)
```
...
top_counts<-pack_sum %>%
  filter(count>quantile(count,probs=0.99, rm.na=T) )
...
top_counts_sorted<-pack_sum %>%
  filter(unique>quantile(unique,probs=0.99, rm.na=T) ) %>%
  arrange(unique)
